...
def evaluate(dataset, model, modelQ, modelA):
    ...
    for j in xrange(len(x.data)):
        ...
        loss = evaluatorA(xA2, tA2, evaluatorQ.predictor.l1.c)
...
def main(datafiles, params):
    ...
    lm = jsai2016models.RNNLMs(len(vocab), n_units)
    model = L.Classifier(lm)
    model.compute_accuracy = True

    lmQ = jsai2016models.RNNLMs(len(vocab), n_units)
    modelQ = L.Classifier(lmQ)
    modelQ.compute_accuracy = True

    lmA = jsai2016models.JSAI2016DIALOGUEs(len(vocab), n_units, modelQ)
    print('### lmA.__class__=', lmA.__class__)
    # modelA = L.Classifier(lmA)
    modelA = jsai2016models.JSAI2016DIALOGUE_CLASSIFIERs(lmA)
    print('### modelA.__class__=', modelA.__class__)
    modelA.compute_accuracy = True
